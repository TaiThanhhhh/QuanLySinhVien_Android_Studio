...
public class AuthService {
    ...
    public AuthResult login(String mssv, String password, String androidId) {
        ...
        if (u == null) return new AuthResult(AuthResult.Status.INVALID_CREDENTIALS, null);
        if (!userRepo.verifyPassword(u, password)) return new AuthResult(AuthResult.Status.INVALID_CREDENTIALS, null);
        ...
        if (u.isPasswordNeedsReset()) return new AuthResult(AuthResult.Status.NEEDS_PASSWORD_RESET, u);

        return new AuthResult(AuthResult.Status.SUCCESS, u);
    }
}
